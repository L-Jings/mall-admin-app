<template>
  ...
  >
    ...
    <a-form-model-item label=""
        :wrapper-col="{ span: 24}" class="next-btn">
...
export default {
  ...
  created() {
    categoryApi.list().then((res) => { // 获取类目
      this.categoryList = res.data;
    });
  },
  methods: {
    changeCategory(category) {
    //   console.log(category);// 打印的是类目的id
      for (let i = 0; i < this.categoryList.length; i += 1) {
        if (this.categoryList[i].id === category) { // 如果对应的是当前类目项，就存储当前的子类目
